$(document).ready(function(){
	...
	function getPersona(dni){	
		$.ajax({
			async: false,
			url:'./php/api-rest.php',
			dataType:'json',
			type:'post',
			data:{dni:dni}
			})
		.done(function(respuesta){
			cadena = JSON.parse(respuesta);
			if (Array.isArray(cadena)){
				for (var i = 0; i <= cadena.length - 1;  i++) {
					...
					nrodni = cadena[i].dni;
					apellynomb = cadena[i].apellido_nombres;
					es_regular = cadena[i].es_regular;	

					tabla.row.add([nrodni,apellynomb, es_regular, acciones]).draw(true);
				}
			}
			else{
	...
	/*LOGOUT*/
    /*$("#a_link_logout").on("click",function(event){
